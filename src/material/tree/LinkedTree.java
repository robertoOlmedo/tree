...
public class LinkedTree<E> implements Tree<E> {
    ...
    public Position<E> moveSubtree(Position<E> pOrig, Position<E> pDest) throws Exception {
        if (esAncestro(pOrig, pDest)) {
            throw new Exception();
        } else {
            TreeNode<E> origen = checkPosition(pOrig);
            TreeNode<E> destino = checkPosition(pDest);
            TreeNode<E> padreOrigen = checkPosition(origen.getParent());
            padreOrigen.children.remove(pOrig);
    ...
    private boolean esAncestro(Position<E> pOrig, Position<E> pDest) {
        boolean esAncestro = false;
        TreeNode<E> nodo = checkPosition(pDest);
        while (!esAncestro && nodo != null) {
            esAncestro = nodo == pOrig;
            if (!esAncestro) {
                nodo = nodo.getParent();
            }
        }
